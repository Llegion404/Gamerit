...
serve(async (req) => {
  ...
  if (!commentId) {
    return new Response(JSON.stringify({ error: "Invalid Reddit comment URL" }), { status: 400 });
  }
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "Reddit API error", details: e.message }), { status: 500 });
  }
  ...
  return new Response(JSON.stringify({ chain_length, prize }), { status: 200 });
});

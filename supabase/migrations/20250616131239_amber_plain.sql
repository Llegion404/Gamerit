/*
  # Update Authentication System for Real Reddit OAuth

  1. Database Changes
    - Add Reddit OAuth fields to players table
    - Add welfare system tracking
    - Add proper indexes for performance
    - Update RLS policies for authenticated users

  2. Security
    - Enable RLS on all tables
    - Add policies for authenticated Reddit users
    - Add welfare system constraints
*/
...
CREATE POLICY "Players can update their own data"
  ...
  FOR UPDATE
  TO authenticated
  USING (reddit_id = auth.jwt() ->> 'reddit_id')
  WITH CHECK (reddit_id = auth.jwt() ->> 'reddit_id');
...
-- Update the place_bet_transaction function to work with reddit_id

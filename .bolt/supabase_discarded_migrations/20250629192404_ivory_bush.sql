/*
  # Fix Meme Market Real-Time Updates

  1. Changes
    - Improve the update_stock_history function to ensure price changes are properly recorded
    - Modify the refresh_meme_stocks function to ensure it's using real data
    - Add a trigger to ensure stock history is properly updated when prices change
    - Fix the threshold for significant changes to ensure more visible price movements
...
BEGIN
  ...
  significant_change := ABS(p_new_value - current_value) > (current_value * 0.01);
  ...
  IF p_new_value != current_value THEN
...
BEGIN
  ...
  IF NEW.current_value != OLD.current_value THEN
    -- Add to history
    PERFORM update_stock_history(NEW.id, NEW.current_value);
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
